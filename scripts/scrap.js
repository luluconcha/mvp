...
async function addSenatorsToDatabases() {
    ...
    await page.goto("https://www.senado.es/web/relacionesciudadanos/participacion/senadores/index.html",
      ...
      const allSenators = document.querySelectorAll("li.alterna three-col, li.three-col")
        ...
        for (let sen of allSenators) {
            ...
            if (email.href.startsWith("mailto:")){
                // router.post("/politicians", insertIntoDB(nameSen.innerText, email.href.substring(7), affiliationSen.title))
                db(`INSERT INTO politicians (name, email_address, party, msgs_sent) VALUES ("${nameSen.innerText}", "${email.href.substring(7)}", "${affiliationSen.title}", 0);`)
            }
        }
    })
